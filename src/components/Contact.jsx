    ...
    >
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-start relative z-10">
        ...
        <div className="space-y-6">
          <ContactCard icon={EmailIcon} label="Email" value={profile.contact.email} href={`mailto:${profile.contact.email}`} />
          <ContactCard icon={PhoneIcon} label="Phone" value={profile.contact.phone} href={`tel:${profile.contact.phone}`} />
          <ContactCard icon={LocationIcon} label="Location" value={profile.contact.location.label} href={profile.contact.location.mapLink} />
        ...
        {/* Right Column - Form */}
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-6 text-black bg-white/5 backdrop-blur-md p-6 rounded-xl border border-white/10 shadow-lg">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input name="name" type="text" placeholder="Your Name" required className="w-full px-4 py-3 rounded-md bg-white/80 border border-white/10 focus:outline-none focus:ring-2 focus:ring-primary" />
            <input name="email" type="email" placeholder="Your Email" required className="w-full px-4 py-3 rounded-md bg-white/80 border border-white/10 focus:outline-none focus:ring-2 focus:ring-primary" />
          </div>
          <input name="title" type="text" placeholder="Subject" required className="w-full px-4 py-3 rounded-md bg-white/80 border border-white/10 focus:outline-none focus:ring-2 focus:ring-primary" />
          <textarea name="message" rows="5" placeholder="Your Message" required className="w-full px-4 py-3 rounded-md bg-white/80 border border-white/10 focus:outline-none focus:ring-2 focus:ring-primary" />
          <button type="submit" className="w-full bg-gradient-to-r from-primary to-accent text-black px-6 py-3 rounded-full font-bold hover:opacity-90 transition-all">
            Send Message
          </button>
          {status && <p className="text-sm text-center pt-2 text-white">{status}</p>}
        </form>
      </div>
    </section>
  );
